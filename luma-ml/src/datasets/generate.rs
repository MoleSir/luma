...
mod tests {
    ...
    use crate::datasets::{make_regression, RegressionOption, RegressionOptionBuilder};

    #[test]
    fn test_make_regression_shapes_and_linearity() {
        let data = make_regression(
            50, 3, &Cpu,
            RegressionOptionBuilder::default().noise(0.0).build().unwrap(),
        ).unwrap();
        ...
        let max_diff = data.y.to_vec().unwrap().into_iter()
            .zip(y_expected.to_vec().unwrap())
            .map(|(a, b)| (a - b).abs())
            .fold(0.0, f64::max);
