...
fn transfer_identity() {
    common::transfer::test_to_device_identity_f32(&Cpu::default());
    common::transfer::test_to_device_identity_int(&Cpu::default());
    common::transfer::test_to_device_identity_bool(&Cpu::default());
    common::transfer::test_to_device_identity_requires_grad(&Cpu::default());
}
...
fn transfer_copy_f32() {
    ...
    let dst = Float::transfer(&src, &Cpu::default()).unwrap();
...
fn transfer_copy_preserves_dtype() {
    let src = tensor_f64_dev(&[1.5, 2.5], (2,), &Cpu::default());
    let dst = Float::transfer(&src, &Cpu::default()).unwrap();
...
fn transfer_copy_non_contiguous_becomes_contiguous() {
    ...
    let dst = Float::transfer(&view, &Cpu::default()).unwrap();
...
fn transfer_copy_int_bool() {
    ...
    let dst = Int::transfer(&src, &Cpu::default()).unwrap();
    assert_ne!(dst.id(), src.id());
    assert_eq!(dst.dtype(), src.dtype());
    assert_eq!(dst.to_vec().unwrap(), vec![1, 2, 3]);

    let srcb = tensor_bool_dev(&[true, false, true], (3,), &Cpu::default());
    let dstb = luma_tensor::Bool::transfer(&srcb, &Cpu::default()).unwrap();
...
fn transfer_copy_severs_graph_preserves_requires_grad() {
    ...
    let moved = Float::transfer(&y, &Cpu::default()).unwrap();
...
fn transfer_copy_grad_flow_works_after_transfer() {
    ...
    x.set_requires_grad(true);
    let moved = Float::transfer(&x, &Cpu::default()).unwrap();
...
fn transfer_meta_errors() {
    ...
    assert!(meta.is_meta());
    let res = Float::transfer(&meta, &Cpu::default());
