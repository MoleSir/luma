...
pub fn softmax<T: CpuFloat>(x: &[T], layout: &Layout, dim: usize) -> Result<Vec<T>> {
    let reduce_dim_stride = layout.stride()[dim];
    let reduce_dim_size = layout.dims()[dim];
    let mut dst = vec![T::ZERO; layout.element_count()];
    ...
    if layout.is_contiguous() && reduce_dim_stride == 1 {
        ...
        for src_index in collapsed.storage_indices() {
            ...
            let row: Vec<T> = DimArray::new(&x[src_index..], reduce_dim_size, reduce_dim_stride).into_iter().collect();
            let mut out_row = vec![T::ZERO; reduce_dim_size];
...
pub fn rms_norm<T: CpuFloat>(x: &[T], x_l: &Layout, weight: &[T], weight_l: &Layout, eps: T) -> Result<Vec<T>> {
    ...
    let mut out = vec![T::ZERO; x_l.element_count()];
    ...
    if x_l.is_contiguous() && last_dim_stride == 1 {
        ...
        let mut dst_offset = 0usize;
        for src_index in collapsed.storage_indices() {
            let row: Vec<T> = DimArray::new(&x[src_index..], last_dim_size, last_dim_stride).into_iter().collect();
