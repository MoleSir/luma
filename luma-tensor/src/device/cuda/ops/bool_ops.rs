use crate::{BoolOps, Layout, Result, Shape, dtype::{BoolDType, FloatDType, IntDType}};
...
impl BoolOps<Cuda> for Cuda {
    ...
    fn b_from_bool(data: &[bool], device: &Cuda, _dtype: BoolDType) -> Result<CudaBoolStorage> {
    ...
    fn b_reduce_all(x: &CudaBoolStorage, layout: &Layout, dims: &[usize], keepdim: bool) -> Result<(CudaBoolStorage, Shape)> {
        let (out, shape) = launch::launch_multi_reduce_by_kernel_name::<u8>(&x.device, "sall_u8", &kernel::REDUCE, &x.slice, layout, dims, keepdim)?;
    ...
    fn b_reduce_any(x: &CudaBoolStorage, layout: &Layout, dims: &[usize], keepdim: bool) -> Result<(CudaBoolStorage, Shape)> {
        let (out, shape) = launch::launch_multi_reduce_by_kernel_name::<u8>(&x.device, "sany_u8", &kernel::REDUCE, &x.slice, layout, dims, keepdim)?;
        Ok((CudaBoolStorage { slice: out, device: x.device.clone() }, shape))
    }

    fn b_true_count(x: &CudaBoolStorage, layout: &Layout) -> Result<usize> {
        let all_dims: Vec<usize> = (0..layout.dims().len()).collect();
        let (result, _) = launch::launch_multi_reduce_by_kernel_name::<u8>(
            &x.device, "ssum_u8", &kernel::REDUCE,
            &x.slice, layout, &all_dims, false,
        )?;
        let val = x.device.memcpy_dtov(&result)?;
        Ok(val[0] as usize)
    }
        ...
        if dim == 0 {
            ...
            for (storage, layout) in srcs {
                ...
                if layout.is_contiguous() {
                    launch::launch_copy2d(device, "ucopy2d_u8", &kernel::COPY, d1, d2, d2, dst_s, &storage.slice, layout.start_offset(), &out)?;
                } else {
    ...
    fn b_pick(mask: &CudaBoolStorage, mask_l: &Layout, on_true: &CudaBoolStorage, true_l: &Layout, on_false: &CudaBoolStorage, false_l: &Layout) -> Result<CudaBoolStorage> {
        let device = &mask.device;
        let slice = launch::launch_pick(device, "u8", &kernel::PICK, &mask.slice, mask_l, &on_true.slice, true_l, &on_false.slice, false_l)?;
        Ok( CudaBoolStorage { slice, device: device.clone() } )
    }

    fn b_pick_true(mask: &CudaBoolStorage, mask_l: &Layout, value: bool, on_false: &CudaBoolStorage, false_l: &Layout) -> Result<CudaBoolStorage> {
        let device = &mask.device;
        let slice = launch::launch_pick_true(device, "u8", &kernel::PICK, &mask.slice, mask_l, value as u8, &on_false.slice, false_l)?;
        Ok( CudaBoolStorage { slice, device: device.clone() } )
    }

    fn b_pick_false(mask: &CudaBoolStorage, mask_l: &Layout, on_true: &CudaBoolStorage, true_l: &Layout, value: bool) -> Result<CudaBoolStorage> {
        let device = &mask.device;
