...
pub trait MatmulDTypeKind<D: Device>: DTypeKind<D> + Sized {
    fn matmul_dispatch(lhs: &Self::Storage, lhs_l: &Layout, rhs: &Self::Storage, rhs_l: &Layout, out_shape: &Shape)
    -> crate::Result<Self::Storage>;
}

impl<D: Device> MatmulDTypeKind<D> for Float {
    #[inline]
    fn matmul_dispatch(lhs: &Self::Storage, lhs_l: &Layout, rhs: &Self::Storage, rhs_l: &Layout, out_shape: &Shape) -> crate::Result<Self::Storage> {
