    ...
    B: Batcher<Item = D::Item>,
{
    pub fn new(dataset: D, batcher: B, batch_size: usize, shuffle: bool) -> Self {
        Self { dataset, batcher, batch_size, shuffle }
    }
    ...
    B: Batcher<Item = D::Item>,
{
    ...
    fn into_iter(self) -> Self::IntoIter {
        let indices = self.get_iter_indices();
        DataLoaderIntoIter { loader: self, cursor: 0, indices }
    }
}
    ...
    if begin >= loader.dataset.len() {
        ...
        let mut f = || -> Result<B::Output, DataLoaderError> {
            ...
            for index in begin..end {
                ...
                let item = loader.dataset.get(index).map_err(|e| DataLoaderError::Dataset(e.to_string()))?.expect("must valid item");
                items.push(item);
            }
            *cursor += items.len();
            let batch = loader.batcher.batch(items).map_err(|e| DataLoaderError::Batcher(e.to_string()))?;
