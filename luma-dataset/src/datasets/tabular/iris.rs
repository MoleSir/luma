...
use crate::{Batcher, DataLoader, Dataset, common::VecDataset, utils};
...
#[derive(Clone)]
pub struct IrisBatch<D: Device> {
    pub features: FloatTensor<D>,
    pub targets: IntTensor<D>,
}

#[derive(Default)]
pub struct IrisBatcher<D: Device> {
    pub device: D,
    _data: PhantomData<D>,
}

impl<D: Device> IrisBatcher<D> {
    pub fn new(device: D) -> Self {
        Self { device, _data: Default::default() }
    }
}

impl<D: Device> Batcher for IrisBatcher<D> {
    type Item = IrisItem;
    type Output = IrisBatch<D>;
    type Error = IrisError;

    fn batch(&self, items: Vec<IrisItem>) -> IrisResult<IrisBatch<D>> {
        let mut features_vec = vec![];
        let mut targets_vec = vec![];

        for item in items.iter() {
            let feature = FloatTensor::new(&item.features, &self.device)?;
        ...
        let targets = IntTensor::new(targets_vec, &self.device)?.reshape((len, 1))?;

        Ok(IrisBatch { features, targets })
    }
}

pub type IrisDataLoader<D> = DataLoader<IrisDataset, IrisBatcher<D>>;
